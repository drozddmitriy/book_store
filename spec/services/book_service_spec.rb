RSpec.describe BookService do
  ...
  context 'when select category' do
    it { expect(book_service.new(category.id, sort_params).sort_for_books.first).to eq(book_by_category) }
  end

  context 'when not select category' do
    it { expect(book_service.new(nil, sort_params).sort_for_books.first).to eq(book) }
  end
end
